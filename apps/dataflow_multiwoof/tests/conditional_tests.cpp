#include "../dfinterface.h"
#include "test_utils.h"
#include "tests.h"

#include <iostream>
#include <string>
#include <vector>

#include <unistd.h>

void selector_test() {
    TEST("Selector test");

    int ns = 1;
    std::string p = "laminar-" + std::to_string(ns);

    add_node(ns, 1, SEL); // a or b

    add_operand(ns, 2); // Selector (0 or 1)
    add_operand(ns, 3); // a
    add_operand(ns, 4); // b
    
    subscribe(ns, 1, 0, ns, 2); // Selector --> SEL:0
    subscribe(ns, 1, 1, ns, 3); // a --> SEL:1
    subscribe(ns, 1, 2, ns, 4); // b --> SEL:2

    setup(ns);

    for (int i = 1; i <= 2; i++) {
        operand op(i - 1, i);   // Selector
        woof_put(p + ".output.2", "output_handler", &op);
        op.value = 10;          // a
        woof_put(p + ".output.3", "output_handler", &op);
        op.value = 20;          // b
        woof_put(p + ".output.4", "output_handler", &op);
    }
    
    do {
        usleep(1e5);
    } while (woof_last_seq(p + ".output.1") < 2);

    operand result;
    woof_get(p + ".output.1", &result, 1);
    ASSERT_EQ(result.value, 10, "SEL = 0 -> result = a = 10");

    woof_get(p + ".output.1", &result, 2);
    ASSERT_EQ(result.value, 20, "SEL = 1 -> result = b = 20");
    
    END_TEST();
}

void filter_test() {
    TEST("Filter test");

    int ns = 1;
    std::string p = "laminar-" + std::to_string(ns);

    add_node(ns, 1, FILTER);

    add_operand(ns, 2); // Filter condition
    add_operand(ns, 3); // Value

    subscribe(ns, 1, 0, ns, 2);
    subscribe(ns, 1, 1, ns, 3);

    setup(ns);

    operand filter1(0.0, 1);
    operand data1(1.0, 1);
    woof_put(p + ".output.2", "output_handler", &filter1);
    woof_put(p + ".output.3", "output_handler", &data1);

    operand filter2(1.0, 2);
    operand data2(2.0, 2);
    woof_put(p + ".output.2", "output_handler", &filter2);
    woof_put(p + ".output.3", "output_handler", &data2);

    operand filter3(1.0, 3);
    operand data3(3.0, 3);
    woof_put(p + ".output.2", "output_handler", &filter3);
    woof_put(p + ".output.3", "output_handler", &data3);

    operand filter4(0.0, 4);
    operand data4(4.0, 4);
    woof_put(p + ".output.2", "output_handler", &filter4);
    woof_put(p + ".output.3", "output_handler", &data4);

    operand filter5(1.0, 5);
    operand data5(5.0, 5);
    woof_put(p + ".output.2", "output_handler", &filter5);
    woof_put(p + ".output.3", "output_handler", &data5);

    do {
        usleep(1e5);
    } while (woof_last_seq(p + ".output.1") < 3);

    std::vector<operand> v;
    unsigned long last = woof_last_seq(p + ".output.1");
    for (unsigned long i = 1; i <= last; i++) {
        woof_get(p + ".output.1", &data1, i);
        v.push_back(data1);
    }

    // // Expected: 2 3 5
    // std::cout << "OUTPUTS: ";
    // for (auto& i : v) {
    //     std::cout << i.value << " ";
    // }

    // Note: duplicates are allowed
    std::vector<unsigned long> expected = {NULL, NULL, 2, 3, NULL, 5};
    unsigned long prev_seq = 1;
    for (auto& op : v) {
        ASSERT(op.seq >= prev_seq, "Sequence increases monotonically");
        ASSERT_EQ(op.value, expected[op.seq], "Ensure value is correct for each sequence number");
        prev_seq = op.seq;
    }

    END_TEST();
}

void conditional_tests() {
    selector_test();
    filter_test();
}