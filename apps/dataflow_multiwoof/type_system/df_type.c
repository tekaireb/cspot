...
int is_number(const DF_VALUE *value) {
    return value->type == DF_BYTE || value->type == DF_SHORT || value->type == DF_INTEGER || value->type == DF_LONG ||
           value->type == DF_UNSIGNED_BYTE || value->type == DF_UNSIGNED_SHORT || value->type == DF_UNSIGNED_INTEGER ||
           value->type == DF_UNSIGNED_LONG || value->type == DF_DOUBLE;
}
